        ...
        <div class="section-header">
          <div>
            ...
            ${
               ...
               * 甲基化两张卡片也可以印基线里的值 —— 管理员照着电话里
               * 念的报告代填的，或患者自己填的 —— 卡片上带着自己的来源
               * 括号。少了这个限定，横幅就会压在医生看得见的数字上面。
               */
              summary.diagnosis.confirmation === 'genetic'
                ? ''
                : `<p class="unconfirmed-banner">${
                    summary.diagnosis.confirmation === 'admin_entered'
                      ? '⚠ 未经基因确诊：本节里没有从基因报告里读出来的 D4Z4 重复数、4q 单倍型或 EcoRI 片段，且档案里的「确诊年份」不是患者本人填写的（详见本节末尾的字段来源），请勿据此确认诊断。'
                      : summary.diagnosis.confirmation === 'self_reported'
                        ? '⚠ 未经基因确诊：本节里没有从基因报告里读出来的 D4Z4 重复数、4q 单倍型或 EcoRI 片段，本节内容不构成诊断依据，请勿据此确认诊断。'
                        : // 不写「本节为空」：这个状态只表示没有分型、没有诊断
                          // 日期；甲基化不在这两项里，没跟着分型或诊断日期一
                          // 起进来的 D4Z4 重复数也不在，两张卡片上都可能有值。
                          '⚠ 尚无诊断依据：本节没有可展示的分型或诊断日期，也没有从基因报告里读出来的基因结果，请勿据此确认诊断。'
                  }</p>`
            }
          </div>
