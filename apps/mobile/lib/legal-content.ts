...
export const GUARDIAN_CONSENT_SECTIONS: LegalSection[] = [
  ...
  {
    title: '4. 谁能看到',
    body: '登录该账号的人，以及我们的管理员账号。这一条在 2026-08-13 这一版改过：以前它写的是「我们的运维人员不会主动查阅具体患儿的报告」，管理员后台上线后这句话不再成立。管理员可以查阅这位患儿的档案，也可以代填基线临床字段（姓名、称呼、所在地区、出生年份、确诊年份、家族史、起病部位、备注）；患儿对自己身体的那些回答，后台只能看，服务端会拒绝代填。代填过的字段会标注「管理员代填」，临床护照和三种导出文件里都单独列出来，不会写成「本人填写」；监护人在「我的 → 编辑资料」里自己再改一次那一个字段，标记就消失、回到监护人名下。称呼、备注只在管理员后台有输入框，App 里没有，监护人在 App 内改不了它们，也去不掉它们的标记，要改值或者去掉标记请按《隐私政策》第 1 条的邮箱或电话找我们。管理员把某个字段清空则不留标记——那里已经没有值可以标了，但清空这个动作记在访问记录里。每一次查阅和修改都会留下带时间的记录，保存 180 天，监护人可以按《隐私政策》第 1 条的方式来问是谁看过。管理员权限不能自助获取，只能由能登录我们服务器的人在命令行上授予。详见《隐私政策》第 10 条。第三方大模型服务的处理需要监护人在首次上传报告前另行单独同意，未同意则报告不会离开我们的服务器。',
  },
  {
    title: '5. 保存多久',
...
export const USER_AGREEMENT_SECTIONS: LegalSection[] = [
  ...
  },
  {
    ...
    body: '我们可能新增、调整或下线功能。你可以随时在「我的 → 注销账号」申请注销：申请后有 7 天冷静期，期间可随时撤销；冷静期结束后我们会删除你的账号及其名下的档案、测试记录、报告文件。详见《隐私政策》第 7 条。',
  },
  {
...
export const PRIVACY_POLICY_SECTIONS: LegalSection[] = [
  ...
  },
  {
    title: '7. 你的权利以及在哪里行使',
    body: [
      '· 查阅、复制、可携带——「我的 → 导出我的数据」一键导出你全部档案、测试记录与报告清单的结构化文件；「临床护照」可导出可打印的就诊摘要。',
  ...
  },
  {
    ...
    body: [
      ...
      '',
      '（四）能改什么，以及改过的会被标出来',
      '管理员只能编辑这些基线临床字段：姓名、称呼、所在地区、出生年份、确诊年份、家族史、起病部位、备注。你对自己身体的那些回答——诊断进展、能不能独立行走、各项困难评分——后台只能看，不能替你填。这不是「后台页面上没做输入框」：服务端有一份名单，改动名单以外的任何字段，请求会被直接拒绝并告诉操作者拒绝了哪几项。',
      '被管理员填过的字段会单独标一个「管理员代填」，标记跟值一起存在你的档案里。你的临床护照（App 里、导出的 PDF、以及你分享给医生的那个网页）会把这些字段单独列出来，写明是管理员在什么时候代填的；你带给医生的转诊资料，以及三种导出文件（FHIR / Phenopacket / TREAT-NMD），也各自带着这份清单。这些地方都不会写成「本人填写」。',
      '你自己再改一次同一个字段，这个标记就消失，这个字段回到你名下——是按字段算的，你改哪一个就只放开哪一个，其它字段的标记不受影响；你打开表单看一眼、原样保存，什么都不会放开。',
      '能这样收回的，是「我的 → 编辑资料」这张表单上有的那些字段。称呼、备注只在管理员后台有输入框，App 里没有，你在 App 内改不了它们，也去不掉它们的标记；要改值或者去掉标记，请按第 1 条的邮箱或电话找我们——我们代你改，标记仍然是「管理员代填」，我们把它清空，那一项就不再有标记（下一段）。',
