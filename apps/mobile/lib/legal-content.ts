...
export const GUARDIAN_CONSENT_SECTIONS: LegalSection[] = [
  ...
  {
    title: '4. 谁能看到',
    body: '登录该账号的人，以及我们的管理员账号。这一条在 2026-08-13 这一版改过：以前它写的是「我们的运维人员不会主动查阅具体患儿的报告」，管理员后台上线后这句话不再成立。管理员可以查阅这位患儿的档案，也可以代填基线临床字段（姓名、称呼、地区、出生与确诊年份、分型、D4Z4、单倍型、甲基化、家族史、起病部位、备注这十二项）；患儿对自己身体的那些回答，后台只能看，服务端会拒绝代填。代填过的字段会标注「管理员代填」，临床护照和三种导出文件里都单独列出来，不会写成「本人填写」；监护人在「我的 → 编辑资料」里自己再改一次那一个字段，标记就消失、回到监护人名下。称呼、单倍型、甲基化、备注只在管理员后台有输入框，App 里没有，监护人在 App 内改不了它们，也去不掉它们的标记，要改值或者去掉标记请按《隐私政策》第 1 条的邮箱或电话找我们。管理员把某个字段清空则不留标记——那里已经没有值可以标了，但清空这个动作记在访问记录里。每一次查阅和修改都会留下带时间的记录，保存 180 天，监护人可以按《隐私政策》第 1 条的方式来问是谁看过。管理员权限不能自助获取，只能由能登录我们服务器的人在命令行上授予。详见《隐私政策》第 10 条。第三方大模型服务的处理需要监护人在首次上传报告前另行单独同意，未同意则报告不会离开我们的服务器。',
  },
  {
    title: '5. 保存多久',
...
export const PRIVACY_POLICY_SECTIONS: LegalSection[] = [
  ...
  },
  {
    ...
    body: [
      ...
      '能这样收回的，是「我的 → 编辑资料」这张表单上有的那些字段。称呼、单倍型、甲基化、备注只在管理员后台有输入框，App 里没有，你在 App 内改不了它们，也去不掉它们的标记；要改值或者去掉标记，请按第 1 条的邮箱或电话找我们——我们代你改，标记仍然是「管理员代填」，我们把它清空，那一项就不再有标记（下一段）。',
