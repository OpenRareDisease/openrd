...
> ## ⚠️ 交付渠道现状（2026-08 核对，v2.5.0）
>
> **本文多处按「可安装的原生 App + 应用商店分发」来描述产品形态。仓库目前产不出原生包。**
>
> 当前唯一能构建、也是唯一在生产上跑着的产物是 **Expo web export**：`apps/mobile/Dockerfile.web` 执行 `npx expo export --platform web`，产物由 nginx 容器提供、Caddy 反代到 `https://ai.fshdyouth.com/`。患者拿到新版本的方式是**刷新页面**。
>
> 具体缺什么：
>
> - 没有 `apps/mobile/android` / `apps/mobile/ios` 原生目录，也从未 `expo prebuild` 过
> - `app.json` 没有 `extra.eas.projectId`，仓库没有链接到任何 EAS project
> - `eas-cli` 和 `expo-updates` 都不在依赖里（也就是说，即便有了原生包，也没有 OTA 通道，每个客户端修复都要走一轮商店审核）
> - 商店签名密钥、开发者账号、应用备案 / 软著材料均未落实
>
> v2.5.0 新增的 `apps/mobile/eas.json` 只是把版本号策略写进仓库（`appVersionSource: local`，production profile 开 `autoIncrement`），`submit` 段**故意留空**——签名和账号未定，凭空写一个 submit 配置会读起来像已经验证过。`app.json` 也补了 `bundleIdentifier` / `package`（`com.fshdyouth.jiyutong`）和 `buildNumber` / `versionCode`，这些是原生分发的**前置条件**，不是它已经存在的证据。
>
> **因此：不要基于本文为「上架 App Store / 各安卓商店」排期或做预算。** 启动原生分发是一项独立立项的工作（prebuild、签名、商店合规材料、渠道包、审核周期、以及一条 OTA 通道，否则每个 hotfix 都要重走审核）。在那之前，下文所有「下载 App」「应用商店」「App 版本号」的表述，都请读作「打开网页」「浏览器」「web bundle 版本」。
>
> 相关：[v2.5.0 部署手册 §3.2 step 6](../runbooks/v2.5.0-deploy.md)、[发布清单 §5](../release-checklist.md)。
...
- **目标终端（本文的规划前提）:** 移动端App (iOS & Android)
- **当前实际终端:** 移动端 **Web**（Expo web export，响应式，在手机浏览器里使用）。见文首的「交付渠道现状」——原生包目前构建不出来，本节的 App 描述是目标而不是现状。
...
| ------------ | -------------------------- | --------------------------------------------- | ---------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------- |
| **初次接触** | 了解FSHD，寻找管理工具     | 下载App，注册登录，浏览首页                   | 引导注册，展示核心功能，提供新手引导           | 信息碎片化，不知如何开始             | 目标：App Store / 安卓商店；**当前实际：分享链接打开 `ai.fshdyouth.com`** |
